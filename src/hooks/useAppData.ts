...
export function useAppData(){ const [user,setUser]=useState<User>(); const [loading,setLoading]=useState(true); const [reports,setReports]=useState<Report[]>([]); const [tips,setTips]=useState<Tip[]>([]); const [alerts,setAlerts]=useState<Alert[]>([]); const [wanted,setWanted]=useState<Wanted[]>([]); const [users,setUsers]=useState<User[]>([]); const [publicReports,setPublicReports]=useState<Report[]>([]); const [publicWanted,setPublicWanted]=useState<Wanted[]>([])
 ...
 // Loaded via dynamic import instead of a static top-level `import ... from
 // '../services'` so that pages which never need Firebase — every public/
 // unauthenticated route (Splash, Welcome, Sign In, Public Portal, About) —
 // don't have the ~600 KB Firebase SDK chunk (auth+firestore+storage+
 // functions) forced onto their initial critical rendering path just
 // because the app root's very first effect needs one function from the
 // same file. `services.ts` itself is completely unchanged; only how this
 // one call site pulls it in. See LIGHTHOUSE_PRODUCTION_REPORT.md.
 useEffect(()=>{ let cancelled=false; const cancelIdle=whenIdle(()=>{ import('../services').then(mod=>{ if(!cancelled) setServices(mod) }) }); return ()=>{cancelled=true; cancelIdle()} },[])
 useEffect(()=>{ if(!services) return; return services.observeAuth(async fb=>{ if(fb){ const existing=await services.profile(fb.uid); const defaultUser:User=existing??{id:fb.uid,name:fb.displayName||fb.email?.split('@')[0]||'TraceNet User',email:fb.email||'',role:'CITIZEN',isApproved:true,status:'ACTIVE',reputationScore:100}; if(!existing) await services.saveProfile(defaultUser); setUser(defaultUser) }else setUser(undefined); setLoading(false)}) },[services])
 useEffect(()=>{
   if(!services) return
   ...
   const stops=[services.collections.alerts(setAlerts),services.collections.publicReports(setPublicReports),services.collections.publicWanted(setPublicWanted)]
   ...
   if(isPrivileged) stops.push(services.collections.allReports(setReports),services.collections.allTips(setTips),services.collections.allWanted(setWanted))
   else if(user) stops.push(services.collections.myReports(user.id,setReports),services.collections.myTips(user.id,setTips))
   if(user?.role==='ADMIN') stops.push(services.collections.allUsers(setUsers))
